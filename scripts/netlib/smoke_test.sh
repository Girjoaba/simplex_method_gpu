#!/bin/bash
# Smoke test script for benchmarking setup
# Run from: ~/simplex_method_gpu

set -euo pipefail

# Activate Python venv for gurobipy
source .venv/bin/activate

LOG_DIR="test/netlib/smoke_test_logs"
mkdir -p "$LOG_DIR"
TIMESTAMP=$(date +%Y%m%d_%H%M%S)

echo "=== SMOKE TEST START: $TIMESTAMP ===" | tee "$LOG_DIR/summary_$TIMESTAMP.log"

# --- Phase 1: Setup ---
echo "[Phase 1] Setting up problem set..."
cd scripts/netlib
cp problem_summary.csv problem_summary.csv.bak
cp problem_summary.csv.bak problem_summary.csv
cd ../..

# --- Phase 2: Prepare all 69 problems ---
echo "[Phase 2] Preprocessing all 69 problems..."
bash scripts/netlib/prepare_problems.sh 2>&1 | tee "$LOG_DIR/prepare_$TIMESTAMP.log"

# --- Phase 3: Build solvers ---
echo "[Phase 3] Building solvers..."
make clean && make 2>&1 | tee "$LOG_DIR/build_$TIMESTAMP.log"

# --- Phase 4: Interface test (all solvers, 1 problem) ---
echo "[Phase 4] Interface test: All solvers on AFIRO..."

# Create single-problem test file
echo 'Name,Rows,Cols,Nonzeros,Bytes,BR,Optimal Value
AFIRO,28,32,88,794,,-4.6475314286E+02' > scripts/netlib/problem_summary.csv

for solver in tp_v1_baseline tp_v2_dense_solve bm_v5_thrust_max_elem bm_v8_full_gpu bm_v10_sherman_morris_opt bm_v11_sparse; do
    echo "  Testing $solver..."
    scripts/netlib/solve_and_compare.sh "bin_solver/${solver}.out" 200000 2>&1 \
        | tee "$LOG_DIR/interface_${solver}_$TIMESTAMP.log"
done

# # --- Phase 5: Problem test (1 solver, all 69 problems) ---
# echo "[Phase 5] Problem test: tp_v1 on all 69 problems..."
# cp scripts/netlib/problem_summary.csv.bak scripts/netlib/problem_summary.csv
#
# scripts/netlib/solve_and_compare.sh bin_solver/tp_v1_baseline.out 200000 2>&1 \
#     | tee "$LOG_DIR/problems_tp_v1_$TIMESTAMP.log"
#
# # --- Cleanup ---
# echo "[Cleanup] Restoring original problem_summary.csv..."
# cp scripts/netlib/problem_summary.csv.bak scripts/netlib/problem_summary.csv

echo "=== INTERFACE TEST COMPLETE ===" | tee -a "$LOG_DIR/summary_$TIMESTAMP.log"
echo "Logs saved to: $LOG_DIR/"
