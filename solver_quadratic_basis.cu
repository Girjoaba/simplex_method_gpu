...
__host__ __device__ __forceinline__ constexpr int AT(int i, int j, int s) { return i * s + j; }

__host__ __device__ __forceinline__ constexpr int R2C(int i, int j, int m) { return i + j * m; }

__host__ __device__ __forceinline__ constexpr int num_blocks_1D(int n) { return (n + BS_1D - 1) / BS_1D; }
...
void print_matrix(real* A, int m, int n, const char* msg = "")
{
    // copy to host
    real* h_A;
    cuda_malloc_host(h_A, m * n);
    cudaMemcpy(h_A, A, m * n * sizeof(real), cudaMemcpyDeviceToHost);
    printf("%s\n", msg);

    for (int i = 0; i < m; ++i) {
        for (int j = 0; j < n; ++j) {
            printf("%f ", h_A[R2C(i, j, m)]);
        }
        printf("\n");
    }
}
...
std::pair<real, SolveStatus> solve(real* A, real* b, real* c, real* x_b, int* b_ixs, int m, int n, TimePoints& t)
{
    ...
    if (cublasCreate(&handle) != CUBLAS_STATUS_SUCCESS) {
        ...
        // print everything there is about the error
        std::cerr << "Error code: " << cublasCreate(&handle) << '\n';
        std::exit(EXIT_FAILURE);
    }
    ...
    do {
        ...
        printf("Iteration %d\n", i);
        print_matrix(d_B_inv, m, m, "B_inv:"); // debug
        t.ev_start = Clock::now();
        // y = c_b * B_inv
        cublasSgemm(handle, CUBLAS_OP_N, CUBLAS_OP_N,
            1, m, m, &alpha, d_c_b, 1, d_B_inv, m, &beta, d_y_aug + 1, 1);

        // print_matrix(d_y, 1, m, "y:"); // debug

        // y_aug[1..m] = y
        // cudaMemcpy(d_y_aug + 1, d_y, m * sizeof(real), cudaMemcpyDeviceToDevice);
        print_matrix(d_y_aug, 1, m + 1, "y_aug:"); // debug
        // e = [1 y] * [-c; A]
        cublasSgemm(handle, CUBLAS_OP_N, CUBLAS_OP_N,
            1, n, m + 1, &alpha, d_y_aug, 1, d_D, m + 1, &beta, d_e, 1);

        print_matrix(d_e, 1, n, "e:"); // debug

        t.blas_end = Clock::now();

        p = entering_var(d_e, n, blocks_for_n, d_curr, d_next, d_ix);
        std::cout << "Iteration " << i << ", entering var index: " << p << '\n';
        t.ev_end = Clock::now();
        if (p < 0) {
