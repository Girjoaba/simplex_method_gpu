...
# A /= col_scales
# c /= col_scales[:n + n_surplus + n_slack]

# =================== Step 7 ===================

# Sparsify A and save the LP problem to a text file

A_sparse = scipy.sparse.csc_matrix(A)

h_values  = A_sparse.data
h_indices = A_sparse.indices
h_starts  = A_sparse.indptr
nnz = len(h_values)

with open(output_file, "w") as f:
    f.write(f"{m} {n} {nnz} {n_surplus} {n_slack} {repr(offset)}\n")

    f.write(" ".join(repr(float(x)) for x in h_values) + '\n')
    f.write(" ".join(str(x) for x in h_indices) + '\n')
    f.write(" ".join(str(x) for x in h_starts) + '\n')
